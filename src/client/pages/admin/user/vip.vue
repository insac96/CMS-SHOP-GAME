<template>
  <UiContent title="VIP Upgrade" sub="Quản lý giao dịch nâng VIP">
    <UiFlex class="mb-4">
    ...
    <UModal v-model="modal.order">
      <ServiceVipView :fetch-id="stateOrder" />
    </UModal>
...
const getList = async () => {
  try {
    ...
    const data = await useAPI('vip/admin/order', JSON.parse(JSON.stringify(page.value)))
...
const successAction = async () => {
  try {
    ...
    await useAPI('vip/admin/verify', JSON.parse(JSON.stringify(stateSuccess.value)))
...
const refuseAction = async () => {
  try {
    ...
    await useAPI('vip/admin/verify', JSON.parse(JSON.stringify(stateRefuse.value)))
...
const waitingAction = async () => {
  try {
    ...
    await useAPI('vip/admin/verify', JSON.parse(JSON.stringify(stateWaiting.value)))
